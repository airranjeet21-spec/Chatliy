  ...
  return (
    <>
      <div className="group flex items-start justify-end gap-[10px] relative mb-4 px-2">
        ...
        <div 
          ref={scrollRef}
          className="w-fit max-w-[75%] sm:max-w-[60%] bg-[#20c7ff] p-3 text-white rounded-tr-none rounded-2xl shadow-md flex flex-col h-auto"
        >
          ...
          {replyTo && (replyTo.text || replyTo.img) && (
            <div className="bg-black/10 border-l-4 border-white p-2 rounded-lg mb-2 text-xs flex justify-between items-center gap-2 min-w-[120px]">
              ...
              {replyTo.img && (
                <img src={replyTo.img} className="w-8 h-8 object-cover rounded-md opacity-80" alt="reply" />
              )}
            </div>
          )}
          ...
          {image && (
            <img 
              src={image} 
              className="w-full max-h-[300px] object-cover rounded-lg mb-1 cursor-pointer hover:brightness-90 transition-all shadow-sm" 
              onClick={() => setShowFullImage(true)} 
              alt="sent"
            />
          )}

          {/* Text Message - Auto Expand & Break Words */}
          {message && (
